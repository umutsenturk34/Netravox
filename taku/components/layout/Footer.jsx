...
export default function Footer({ company }) {
  ...
  const waUrl    = cleanNum ? `https://wa.me/${cleanNum}?text=${encodeURIComponent('Merhaba, ürünleriniz hakkında bilgi almak istiyorum.')}` : null;
    ...
    <footer className="bg-[#0A0A0A] text-white">
      ...
      <div className="border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <p className="text-xs tracking-[0.2em] uppercase mb-1" style={{ color: 'var(--color-accent)' }}>E-Bülten</p>
          ...
          <form className="flex w-full md:w-auto gap-0" onSubmit={(e) => e.preventDefault()}>
            ...
            <button
              type="submit"
              className="text-[#0A0A0A] text-xs font-bold tracking-[0.1em] uppercase px-6 py-3 hover:bg-white transition-colors"
      ...
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-2 md:grid-cols-4 gap-10">
          ...
          <div className="flex gap-4 mt-5">
            {social.instagram && (
              <a href={social.instagram} target="_blank" rel="noopener noreferrer"
                className="text-white/40 hover:text-[#C8A882] transition-colors text-xs tracking-wider uppercase">
                Instagram
              </a>
            )}
            {social.tiktok && (
        ...
        <div>
          ...
          <ul className="space-y-3">
            {['T-Shirts', 'Hoodies', 'Dış Giyim', 'Aksesuar'].map((item) => (
              <li key={item}>
                <Link href="/koleksiyonlar" className="text-sm text-white/60 hover:text-white transition-colors">
                  {item}
                </Link>
              </li>
            ))}
        ...
        {/* İletişim */}
        <div>
          <p className="text-[10px] tracking-[0.2em] uppercase text-white/30 mb-4">İletişim</p>
          <ul className="space-y-3 text-sm text-white/60">
            {waUrl && (
              <li>
                <a href={waUrl} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 font-medium hover:text-[#1ebe5d] transition-colors"
                  style={{ color: '#25D366' }}>
            ...
            {contact.address && (
              <li className="leading-snug">{contact.address}{contact.city ? `, ${contact.city}` : ''}</li>
            )}
          </ul>
        </div>
      </div>
      ...
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="text-white/25 text-xs">© {year} {name}. Tüm hakları saklıdır.</p>
