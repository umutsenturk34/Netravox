  ...
  return (
    <>
      <div className="bg-[#F0FDFF] py-16 text-center border-b border-[#E0F2FE]">
        <p className="text-xs tracking-[0.3em] uppercase text-[#0E7490] font-semibold mb-3">Bize Ulaşın</p>
        <h1 className="text-4xl text-[#0C1B2E]">İletişim</h1>
      </div>

      <div className="max-w-5xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-14">
        <div className="space-y-8">
          <div>
            <h2 className="text-2xl text-[#0C1B2E] mb-6">Bilgilerimiz</h2>
            <div className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#F0FDFF] border border-[#E0F2FE] flex items-center justify-center shrink-0">{item.icon}</div>
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-[#0E7490] mb-1">{item.label}</p>
              ...
              <div className="flex flex-wrap gap-3">
                {Object.entries(social).map(([key, url]) => url ? (
                  <a key={key} href={url} target="_blank" rel="noopener noreferrer"
                    className="text-sm px-4 py-2 border border-[#E0F2FE] rounded-xl hover:border-[#0E7490] hover:text-[#0E7490] transition-colors capitalize">{key}</a>
                ) : null)}
              </div>
            </div>
          )}
          {contact.mapUrl ? (
            <div className="rounded-2xl overflow-hidden border border-[#E0F2FE] h-52">
              <iframe src={contact.mapUrl} width="100%" height="100%" style={{ border: 0 }} loading="lazy" title="Harita" />
            </div>
          ) : (
            <div className="rounded-2xl overflow-hidden border border-[#E0F2FE] bg-[#F0FDFF] h-52 flex items-center justify-center">
        ...
        <div>
          <h2 className="text-2xl text-[#0C1B2E] mb-6">Mesaj Gönderin</h2>
          {sent ? (
            <div className="text-center py-12">
              <div className="w-14 h-14 rounded-full bg-[#CFFAFE] flex items-center justify-center mx-auto mb-4 text-xl text-[#0E7490]">✓</div>
              <h3 className="font-bold text-[#0C1B2E] mb-1">Mesajınız İletildi</h3>
              <p className="text-sm text-[#64748B]">En kısa sürede geri dönüş yapacağız.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className={labelClass}>Mesajınız *</label>
                <textarea required value={form.message} onChange={set('message')} rows={5} className={`${inputClass} resize-none`} /></div>
              {error && <p className="text-sm text-red-600 bg-red-50 px-4 py-3 rounded-xl">{error}</p>}
              <div className="flex items-start gap-3">
                <input required type="checkbox" id="kvkk2" checked={form.kvkkConsent} onChange={set('kvkkConsent')} className="mt-0.5 accent-[#0E7490]" />
                <label htmlFor="kvkk2" className="text-xs text-[#64748B] leading-relaxed">
                  <a href="#" className="text-[#0E7490] hover:underline">Kişisel verilerin işlenmesine</a> ilişkin metni okudum. *
                </label>
              </div>
